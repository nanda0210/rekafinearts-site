...
const generateScript = path.join(projectDir, "generate-image-data.py");
...
function getOnlineDbInfo() {
  ...
  if (!dbName && !dbHost) {
    return {
      ...
      note: "Online/cPanel database is not configured in this local manager. Add REMOTE_DB_NAME / REMOTE_DB_HOST in .env if you want to document it here.",
    };
  }
  ...
  return {
    ...
    host: dbHost || "-",
    type: dbType,
    note: "This is metadata only. The current manager does not directly administer the online database.",
  };
}
...
app.get("/api/system/summary", async (req, res) => {
  try {
    ...
    res.json({
      ...
      urls: getUrls(),
      scripts: {
        generateImageData: generateScript,
...
app.post("/api/system/backend/start", async (req, res) => {
  try {
    ...
    if (current.status === "running-external") {
      return res.json({
        ok: true,
        message: "Port 3002 is already in use by another backend process.",
        backend: current,
      });
    }
...
app.post("/api/system/backend/stop", async (req, res) => {
  try {
    ...
    if (!pid || !isProcessRunning(pid)) {
      ...
      return res.json({
        ok: true,
        message: "No managed backend process to stop.",
        backend: current,
      });
    }
    ...
    res.json({
      ok: true,
      message: "Managed backend stop requested.",
      backend: await getBackendStatus(),
    });
  } catch (err) {
...
app.post("/api/system/backend/restart", async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      message: "Managed backend restarted.",
      backend: await getBackendStatus(),
    });
  } catch (err) {
...
app.get("/api/system/db/metrics", async (req, res) => {
  try {
    res.json(await getLocalDbMetrics());
  } catch (err) {
...
app.post("/api/tools/generate-image-data", async (req, res) => {
  try {
    if (!fileExists(generateScript)) {
      return res.status(404).json({ error: "generate-image-data.py not found" });
    }

    await runProcess("python3", [generateScript]);
    res.json({
      ok: true,
      message: "generate-image-data.py completed. It refreshes local image metadata used by the site.",
...
app.post("/api/tools/watermark-images", async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      message: "Watermark applied locally to public/images using 'Reka Fine Arts'. Rebuild and upload changed files to publish.",
      scriptPath: watermarkScript,
      outputTail: tailFile(toolLogFile, 80),
    });
  } catch (err) {
    res.status(500).json({ error: err.message || "Watermark tool failed" });
  }
});
...
export default function DeployManager() {
  ...
  function resetPassword() {
    ...
    setPasswordInput("");
    setFtpStatus("");
    setStatus("cPanel password cleared. Enter it again when ready.");
  }
  ...
  async function runWatermarkTool() {
    await runAction("Applying watermark to local images...", async () => {
      const data = await fetchJson(`${API_BASE}/api/tools/watermark-images`, { method: "POST" });
      setToolOutput(data.outputTail || "");
      setStatus(data.message || "Watermark tool completed.");
    });
  }
  ...
  async function scan(build = true) {
    ...
    setStatus(
      build
        ? "Build + Scan runs locally, then compares local dist against cPanel."
        : "Refresh Changed Files compares current local dist against cPanel and lists only changed files."
    );
    ...
    try {
      const data = await fetchJson(`${API_BASE}/api/deploy/scan`, {
        ...
        body: JSON.stringify({ build, password: activePassword }),
      });
  ...
  async function uploadSelected() {
    ...
    setUploading(true);
    setStatus("Uploading selected local files into the cPanel file structure...");
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-lg text-stone-500">
              Local control center for backend, databases, image tooling, and cPanel uploads.
            </p>
          </div>
          ...
          <div className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-stone-200 lg:w-[420px]">
            ...
            <div className="flex gap-3">
              ...
              {!activePassword ? (
                ...
                <button
                  ...
                  className="rounded-2xl bg-stone-400 px-4 py-3 text-white"
                >
                  Change
                </button>
              )}
            ...
            <p className="mt-2 text-xs text-stone-500">
              After submit, the field is locked for this local session. Use Change to update it.
            </p>
          </div>
        </div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-stone-200">
            ...
            <p className="mt-2 text-xs text-stone-500">
              FTP check should be quick. Build + Scan takes longer because it builds locally and compares remote files.
            </p>
          </div>
        </div>
        ...
        <div className="mb-6 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-stone-200">
          <h2 className="text-2xl font-semibold text-stone-900">How This Page Works</h2>
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl bg-stone-50 p-4">
              <p className="text-sm font-medium text-stone-800">Build + Scan</p>
              <p className="mt-2 text-sm text-stone-600">
                Runs locally, rebuilds dist, then compares local output against cPanel and lists only changed files.
              </p>
            </div>
            <div className="rounded-2xl bg-stone-50 p-4">
              <p className="text-sm font-medium text-stone-800">Refresh Changed Files</p>
              <p className="mt-2 text-sm text-stone-600">
                Does not rebuild. It compares the current local dist folder against cPanel and refreshes the changed file list.
              </p>
            </div>
            <div className="rounded-2xl bg-stone-50 p-4">
              <p className="text-sm font-medium text-stone-800">Upload Selected</p>
              <p className="mt-2 text-sm text-stone-600">
                Uploads only the checked local files into the matching cPanel file structure.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 grid gap-6 xl:grid-cols-2">
          <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-stone-200">
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="rounded-2xl bg-stone-50 p-4">
                ...
                <p className="mt-1 text-sm text-stone-600">Stops only the backend process started by this manager. It does not stop unrelated processes.</p>
              ...
              <div className="rounded-2xl bg-stone-50 p-4">
                ...
                <p className="mt-1 text-sm text-stone-600">Stops the managed backend and starts it again, useful after backend code changes.</p>
              ...
              <div className="rounded-2xl bg-stone-50 p-4">
                ...
                <p className="mt-1 text-sm text-stone-600">SQLite is local only here. Test checks readability. Init creates schema. Seed loads data if your scripts exist.</p>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-stone-200">
            ...
            <div className="mt-5 space-y-4">
              <div className="rounded-2xl bg-stone-50 p-4">
                <p className="text-sm font-medium text-stone-800">generate-image-data.py</p>
                <p className="mt-1 text-sm text-stone-600">
                  Refreshes local image data/metadata used by the site. Run this when image sets change or image mapping needs regeneration.
                </p>
                <div className="mt-3">
                  <ActionButton onClick={runGenerateImageData} color="bg-rose-700" disabled={busy}>Run generate-image-data.py</ActionButton>
                </div>
              </div>

              <div className="rounded-2xl bg-stone-50 p-4">
                <p className="text-sm font-medium text-stone-800">Watermark Local Images</p>
                <p className="mt-1 text-sm text-stone-600">
                  Applies the watermark text “Reka Fine Arts” onto local images in public/images. Rebuild and upload changed files after running it.
                </p>
                <div className="mt-3">
                  <ActionButton onClick={runWatermarkTool} color="bg-stone-900" disabled={busy}>Apply Watermark</ActionButton>
                </div>
              </div>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-stone-200">
            ...
            <div className="mt-5 space-y-5">
              ...
              <div className="rounded-2xl bg-stone-50 p-4">
                ...
                <p className="mt-2 text-sm text-stone-600">
                  {onlineDb?.note || "No online database info configured."}
                </p>
                {onlineDb?.configured && (
                  <div className="mt-3 space-y-2 text-sm text-stone-700">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-stone-500">
                  Build + Scan is local. Refresh compares local dist to cPanel. Upload Selected sends checked local files into cPanel.
                </p>
              </div>

              <div className="flex flex-wrap gap-3">
                <ActionButton onClick={() => scan(true)} color="bg-blue-600" disabled={scanning || uploading}>
                  {scanning ? "Scanning..." : "Build + Scan"}
                </ActionButton>
                <ActionButton onClick={() => scan(false)} color="bg-stone-900" disabled={scanning || uploading}>
                  Refresh Changed Files
                </ActionButton>
                <ActionButton onClick={uploadSelected} color="bg-rose-700" disabled={uploading || scanning || selectedPaths.length === 0}>
...
def main():
    ...
    print("4. Optional for watermark tool: pip3 install pillow")
