...
function sortUploadPaths(paths) {
...
export async function uploadSelectedFiles(selectedPaths = [], ftpPassword) {
  ...
  const withIndex = uniquePaths([
    ...selectedPaths,
    ...(localMap.has("index.html") ? ["index.html"] : []),
  ]);
  ...
  return withClient(async (client) => {
    const uploaded = [];
    const orderedPaths = sortUploadPaths(withIndex);
...
app.post("/api/deploy/upload", async (req, res) => {
  try {
    ...
    res.json({
      uploaded,
      updatedAt: now,
      indexUpdatedAt: uploaded.includes("index.html") ? now : null,
...
export default function DeployManager() {
  ...
  async function scan(build = true) {
    ...
    setProgressLabel(build ? "Build started… Build + Scan in progress." : "Refresh Changed Files in progress.");
    setStatus(
      build
        ? "Build + Scan runs locally, then compares local dist against cPanel by content."
  ...
  async function uploadToCpanel(paths) {
    if (!activePassword) {
      setStatus("Submit the cPanel password first.");
      return;
    }
    ...
    setUploading(true);
    setProgressActive(true);
    setProgressLabel("Uploading to cPanel… index.html will be uploaded last by default.");
    ...
    try {
      const data = await fetchJson(`${API_BASE}/api/deploy/upload`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ files: paths, password: activePassword }),
      });
  ...
  async function uploadSelected() {
    await uploadToCpanel(selectedPaths);
  }

  async function uploadAllChanged() {
    await uploadToCpanel(files.map((f) => f.path));
  }
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-lg text-stone-500">
              Local control center for backend, databases, image tools, and cPanel uploads.
            </p>
          </div>
          ...
          <div className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-stone-200 lg:w-[420px]">
            ...
            <p className="mt-2 text-xs text-stone-500">
              After submit, the field is locked. Use Change to update it.
            </p>
          </div>
        </div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-stone-200">
            ...
            <p className="mt-2 text-xs text-stone-500">
              FTP check should be quick. Hash-based file comparison during scan takes longer.
            </p>
          </div>
        </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl bg-stone-50 p-4">
              ...
              <p className="mt-2 text-sm text-stone-600">
                Builds locally, then compares local dist against cPanel by content hash and lists only real changes.
              </p>
            </div>
            <div className="rounded-2xl bg-stone-50 p-4">
              <p className="text-sm font-medium text-stone-800">Refresh Changed Files</p>
              <p className="mt-2 text-sm text-stone-600">
                Does not rebuild. It compares the current local dist folder against cPanel and lists only changed files.
              </p>
            </div>
            <div className="rounded-2xl bg-stone-50 p-4">
              <p className="text-sm font-medium text-stone-800">Upload to cPanel</p>
              <p className="mt-2 text-sm text-stone-600">
                Uploads the chosen local files into cPanel and always includes index.html last by default.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-stone-200">
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="rounded-2xl bg-stone-50 p-4">
                ...
                <p className="mt-1 text-sm text-stone-600">Adds the “Reka Fine Arts” watermark to local images to reduce exact customer downloads.</p>
              ...
              <div className="rounded-2xl bg-stone-50 p-4">
                ...
                <p className="mt-1 text-sm text-stone-600">Runs Apply Watermark and Refresh Image Data together so the images are ready for Build + Scan.</p>
        ...
        {lastUploaded.length > 0 && (
          <div className="mb-6 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-stone-200">
            ...
            <p className="mt-1 text-sm text-stone-600">index.html updated live at {formatTimestamp(indexUpdatedAt)}</p>
        ...
        <div className="rounded-3xl bg-white shadow-sm ring-1 ring-stone-200">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-stone-500">
                  Build + Scan runs locally. Refresh Changed Files compares local dist to cPanel. Upload to cPanel always includes index.html.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <ActionButton onClick={uploadSelected} color="bg-rose-700" disabled={uploading || scanning || selectedPaths.length === 0}>
                  {uploading ? "Uploading..." : "Upload to cPanel"}
                </ActionButton>
                <ActionButton onClick={uploadAllChanged} color="bg-green-700" disabled={uploading || scanning || files.length === 0}>
                  {uploading ? "Uploading..." : "Upload All Changed"}
                </ActionButton>
              </div>
            </div>
          </div>
          ...
          {files.length === 0 ? (
            <div className="px-6 py-10 text-sm text-stone-500">No changed files to upload.</div>
          ) : (
...
def main():
    ...
    backup(LOCAL_MANAGER)
    backup(DEPLOY_MANAGER)
    if WATERMARK_SCRIPT.exists():
        backup(WATERMARK_SCRIPT)
    ...
    print("\nDone.")
    print("Next:")
    print("1. restart local manager: npm run manager:start")
    print("2. restart frontend if needed: npm run dev")
